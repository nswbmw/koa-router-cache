...
module.exports = function (app, opts) {
  ...
  for (let url in opts) {
    ...
    if ('number' !== typeof urlConf.expire) {
      throw new Error('`expire` must be number!');
    }
  ...
  return function *cache(next) {
    ...
    for (let i in opts) {
      if (opts[i]._compilePathToRegexp.test(method + ' ' + url)) {
        ...
        if (this.status === 200) {
          yield* opts[i].set.call(this, key, this.body, opts[i].expire);
        }
